...
```{r}
library(readr) #paket untuk baca file
```
...
for (i in seq_along(output_for)) {
  output_for[[i]] <- read.csv(daftar_berkas[[i]]) #mengimpor berkas-berkas csv pakai read.csv
}
output_for
```
...
output_for <- Reduce(rbind, output_for) #gambungkan 5 file, jadi 1 datafram
output_for
```
...
```{r}
#cara 2 untuk read csv pakai lapply
daftar_berkas
