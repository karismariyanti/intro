...
Catatan: tabel2 tidak tidy karena cases dan population dijadikan value, seharusnya kolom.
...
#spread vs gahter. spread harus tau nama kolomnya. gather tidak harus tau nama kolomnya
#key kolom yang akan diubah menjadi nama kolom
#
#ubah key adalah kolom dari type, value adalah value dari count
table2_tidy
```
...
```
catatan: tabel4a tidak tidy karena ada data observasi 1999 2000, malah jadi kolom
...
```r
?gather #menampilkan help diawali dengan ?
```
...
```r
#gather mengubah kolom dimana saat kita ngga perlu tahu nama kolomnya, cukup gunakan index. perlu buat kolom baru

gather(table4a, key = "year", value = "case", 2:3) #pakai index
```
...
```r
gather(table4a, key = "year", value = "case", `1999`:`2000`) #atau pakai nama kolom. catatan: kalo pakai angka harus pakai backtick
```
...
```r
gather(table4a, key = "year", value = "case", c(2,3)) #atau c jika tidak berurutan
```
...
table4a_tidy <- table4a %>% 
  gather(key = "year", value = "kasus", 2:3) # menggunakan tidyverse syntax, pipe %>%
table4a_tidy
```
...
```r
#cara 2 pakai intermediate object
table4a_tidy <- gather(table4a, key = "year", value = "kasus", 2:3) 
table4a_tidy
```
...
```r
#mengurutkan data cara 2
mydata <- arrange(mydata, country)
mydata
```
...
```r
#cek kesamaan 2 tabel
identical(table1,mydata) #menghasilkan FALSE, artinya tidak identik dengan tabel 1 karena tipe data year berbeda.
```
...
```r
#perlu kembali lagi kebagian gather, ditambahkan sintax convert = TRUE
...
```r
table4a_tidyy <- gather(table4a, key = "year", value = "kasus", 2:3, convert = TRUE) 
table4a_tidyy
```
...
```r
table4b_tidyy <- gather(table4b, key ="year", value ="population", 2:3, convert = TRUE)
table4b_tidyy
```
...
```r
#join lagi untuk tabel yy
mydata <- left_join(table4a_tidyy, table4b_tidyy, by = c("country", "year"))
mydata
```
...
View(tb1)
#cara lain untuk menampilkan langsung ke layar, tambah kurung diawal dan akhir
(tb1 <- gather(who, key = "key", value = "case", new_sp_m014:newrel_f65))
```
...
View(tb3)
tb4 <- summarise(tb3, cases = sum(case, na.rm = TRUE)) #mengabaikan nilai N/A
View(tb4)
(tb5 <- ungroup(tb4))
```
