...
##     3.88034      0.06664
```
artinya: jika bty_avg meningkat 1 poin, penilaiannya meningkat 0.06
...
```r
summary(model1) #perhitungan kesimpulan: tidak dapat merangkum/eksplain modelnya
```
...
```r
glance(model1) #ditampilkan dalam tabel, hasilnya sama dengan summary(0)
```
...
```r
tidy(model1) #merangkum nilai estimate
```
...
```r
  geom_pointrange(aes(ymax = bty_avg, y = .fitted))
```

```
## mapping: y = ~.fitted, ymax = ~bty_avg 
...
```r
#catatan: dataset tetap terjaga, namun ditambahkan hasil perhitungan
...
broom menampilkan nilai yang ada di model + statistik
model r menampilkan set komplit + prediction = residual
